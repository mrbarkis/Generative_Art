...
def plot_log(ax, track_kT, track_E, kTs, N_cities=233):
    ...
    ax.set(
        xlim=(kT_max, kT_min),
        title='avg. distance between cities vs. available "thermal energy"',
        ylabel="E / N [km]",
        xlabel="kT [km]",
    )
    ax.tick_params(
    ...
    ax.scatter(
        np.asarray(track_kT) / 1000,
        np.asarray(track_E) / N_cities,
        cmap=plt.cm.plasma,
        c=np.asarray(track_kT) / 1000,
        vmin=kT_min,
        vmax=kT_max,
    )
...
def reset(G_start, G_best, seed):
    return (copy.deepcopy(G_start), copy.deepcopy(G_best), RandomState(seed), [], [])
